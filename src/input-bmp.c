...
static int ReadChannelMasks(unsigned long *, Bitmap_Channel *, unsigned int);
static unsigned char *ReadImage(FILE *, int, int, unsigned char[256][3], int, int, int, int, int, const Bitmap_Channel *, at_exception_type *);

at_bitmap input_bmp_reader(char * filename, at_input_opts_type * opts, at_msg_func msg_func, void * msg_data, void * user_data)
{
  FILE *fd;
  ...
  image_storage = ReadImage(fd, Bitmap_Head.biWidth, Bitmap_Head.biHeight, ColorMap, Bitmap_Head.biClrUsed, Bitmap_Head.biBitCnt, Bitmap_Head.biCompr, rowbytes, Grey, masks, &exp);
...
#ifdef _DEBUG
		LOG("Channel %d mask %08x in %d max_val %d\n",
...
static unsigned char *ReadImage(FILE * fd, int width, int height, unsigned char cmap[256][3], int ncols, int bpp, int compression, int rowbytes, int Grey, const Bitmap_Channel * masks, at_exception_type * exp)
{
  unsigned char v, n;
  int xpos = 0;
  int ypos = 0;
