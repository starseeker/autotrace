...
/* Input handler should be implemented with using
  ...
  typedef
  at_bitmap(*at_input_func)(char *name, at_input_opts_type *opts, at_msg_func msg_func, void *msg_data,
                            void *user_data);

/* at_input_add_handler
   Register an input handler to autotrace.
   If a handler for the suffix is already existed, do nothing. */
extern int at_input_add_handler(const char *suffix, const char *description, at_input_func reader);
...
extern int at_input_add_handler_full(const char *suffix, const char *description, at_input_func reader);
...
extern void printInputFormatInfo(FILE *file);
